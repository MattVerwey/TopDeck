...
def discovery_service(mock_loki_collector, mock_prometheus_collector):
    ...
    return MonitoringDependencyDiscovery(
        loki_collector=mock_loki_collector, prometheus_collector=mock_prometheus_collector
    )
...
class TestMonitoringDependencyDiscovery:
    ...
    @pytest.mark.asyncio
    async def test_discover_dependencies_from_logs_http(
        self, discovery_service, mock_loki_collector
    ):
    ...
    @pytest.mark.asyncio
    async def test_discover_dependencies_from_logs_database(
        self, discovery_service, mock_loki_collector
    ):
        ...
        evidence = await discovery_service.discover_dependencies_from_logs(
            resource_ids=["app-service"], duration=timedelta(hours=1)
        )
    ...
    @pytest.mark.asyncio
    async def test_discover_dependencies_from_metrics(
        self, discovery_service, mock_prometheus_collector
    ):
    ...
    async def test_analyze_traffic_patterns(
        self, discovery_service, mock_loki_collector, mock_prometheus_collector
    ):
...
class TestElasticsearchIntegration:
    ...
    def discovery_with_es(self, mock_elasticsearch_collector):
        ...
        return MonitoringDependencyDiscovery(elasticsearch_collector=mock_elasticsearch_collector)
