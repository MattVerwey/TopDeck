...
def test_get_resource_attachments_both_directions(topology_service, mock_neo4j_client):
    ...
    mock_downstream_result.__iter__ = Mock(
        return_value=iter(
            [
                {
                    "source_id": "resource-3",
...
def test_build_attachment_context(topology_service):
    ...
    properties = {"port": 443, "protocol": "https", "endpoint": "https://api.example.com"}
...
def test_get_attachment_analysis(topology_service, mock_neo4j_client):
    ...
    mock_resource_result.single.return_value = {"name": "test-app", "type": "app_service"}
    ...
    mock_chains_down.__iter__ = Mock(
        return_value=iter(
            [
                {
