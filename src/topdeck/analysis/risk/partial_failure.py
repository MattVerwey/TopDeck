...
class PartialFailureAnalyzer:
    ...
    def analyze_degraded_performance(
        ...
        pattern = self.DEGRADATION_PATTERNS.get(
            resource_type.lower(), self.DEGRADATION_PATTERNS["default"]
        )
    ...
    def analyze_partial_outage(
        ...
        overall_impact = (
            ...
            else ImpactLevel.MEDIUM if affected_percentage > 30 else ImpactLevel.LOW
        )
    ...
    def _get_user_impact_description(
        self, outcome_type: OutcomeType, affected_percentage: float
    ) -> str:
    ...
    def _calculate_overall_impact(self, outcomes: list[FailureOutcome]) -> ImpactLevel:
        ...
        weighted_score = sum(
            o.probability
            * severity_weights.get(o.outcome_type, 2.0)
            * (o.affected_percentage / 100)
            for o in outcomes
        )
    ...
    def _generate_degradation_mitigation(
        self, resource_type: str, outcomes: list[FailureOutcome]
    ) -> list[str]:
    ...
    def _generate_monitoring_recommendations(
        self, resource_type: str, pattern: dict[str, Any]
    ) -> list[str]:
        """Generate monitoring recommendations."""
        recommendations = [
