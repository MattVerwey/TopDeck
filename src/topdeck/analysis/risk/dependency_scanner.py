...
class DependencyScanner:
    ...
    def scan_python_dependencies(
        ...
        if requirements_file.exists():
            ...
            vulnerabilities.extend(
                self._check_python_vulnerabilities(dependencies, resource_id)
            )
    ...
    def scan_node_dependencies(
        ...
        if package_json.exists():
            ...
            vulnerabilities.extend(
                self._check_node_vulnerabilities(dependencies, resource_id)
            )
    ...
    def scan_all_dependencies(
        ...
        all_vulnerabilities.extend(
            self.scan_python_dependencies(project_path, resource_id)
        )
        
        # Scan Node.js
        all_vulnerabilities.extend(
            self.scan_node_dependencies(project_path, resource_id)
        )
    ...
    def get_vulnerability_risk_score(
        self,
        ...
        total_score = sum(
            severity_scores.get(v.severity.lower(), 5)
    ...
    def generate_vulnerability_recommendations(
        ...
        if high:
            recommendations.append(
                f"⚠️ HIGH: {len(high)} high-severity vulnerabilities - "
                "schedule urgent upgrades"
            )
