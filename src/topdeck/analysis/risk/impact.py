...
class ImpactAnalyzer:
    ...
    def calculate_blast_radius(self, resource_id: str, resource_name: str) -> BlastRadius:
        """
        ...
        user_impact = self._estimate_user_impact(
            resource_id, directly_affected, indirectly_affected
        )

        # Estimate downtime
        estimated_downtime = self._estimate_downtime(total_affected, user_impact)

        # Get critical path
        critical_path = self.dependency_analyzer.find_critical_path(resource_id)

        # Breakdown affected services by type
        affected_services = self._count_by_service_type(directly_affected + indirectly_affected)
    ...
    def _estimate_user_impact(
        self, resource_id: str, directly_affected: list[dict], indirectly_affected: list[dict]
    ) -> ImpactLevel:
        """
    ...
    def _estimate_downtime(self, total_affected: int, user_impact: ImpactLevel) -> int:
        """
    ...
    def _count_by_service_type(self, resources: list[dict]) -> dict[str, int]:
        """
