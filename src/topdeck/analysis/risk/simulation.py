...
from .models import FailureSimulation, BlastRadius
...
class FailureSimulator:
    ...
    def _calculate_cascade_depth(self, blast_radius: BlastRadius) -> int:
        ...
        max_distance = max(
            r.get("distance", 1)
            for r in blast_radius.indirectly_affected
        )
    ...
    def _generate_recovery_steps(
        ...
        if blast_radius.total_affected > 0:
            steps.append(
                f"8. Monitor and restart {blast_radius.total_affected} dependent services"
            )
