...
class ConnectionStringParser:
    ...
    AZURE_SQL_PATTERN = re.compile(
        r"Server=tcp:([^,;]+)(?:,(\d+))?;.*Database=([^;]+)", re.IGNORECASE
    )
    ...
    MYSQL_PATTERN = re.compile(
        r"(?:mysql://)?(?:([^:]+):([^@]+)@)?([^:/]+)(?::(\d+))?(?:/([^?]+))?", re.IGNORECASE
    )

    # Redis connection string
    REDIS_PATTERN = re.compile(
        r"redis(?:s)?://(?:([^:]+):([^@]+)@)?([^:/]+)(?::(\d+))?(?:/(\d+))?", re.IGNORECASE
    )
    ...
    S3_ENDPOINT_PATTERN = re.compile(
        r"https?://([^.]+)\.s3(?:\.([^.]+))?\.amazonaws\.com", re.IGNORECASE
    )

    # Generic endpoint pattern
    ENDPOINT_PATTERN = re.compile(r"(https?://)?([^:/]+)(?::(\d+))?(/.*)?", re.IGNORECASE)
    ...
    def parse_connection_string(connection_string: str) -> ConnectionInfo | None:
        ...
        if "://" in connection_string or re.match(
            r"^[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,}$", connection_string
        ):
    ...
    def _parse_generic_endpoint(connection_string: str) -> ConnectionInfo | None:
        ...
        try:
            ...
            if "://" in connection_string:
                ...
                if match:
                    protocol = match.group(1).removesuffix("://") if match.group(1) else "https"
                    host = match.group(2)
                    port = (
                        int(match.group(3))
                        if match.group(3)
    ...
    def create_dependency_from_connection(
        source_id: str, target_id: str, conn_info: ConnectionInfo, description: str | None = None
    ) -> ResourceDependency:
        """
        ...
        return ResourceDependency(
            ...
            or f"Connection to {conn_info.service_type} at {conn_info.host}",
        )
