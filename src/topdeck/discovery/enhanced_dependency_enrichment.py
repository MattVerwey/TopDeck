...
class EnhancedDependencyEnrichment:
    ...
    def __init__(
        ...
        self.monitoring_discovery = MonitoringDependencyDiscovery(
            loki_collector=self.loki_collector,
    ...
    async def enrich_resource_dependencies(
        ...
        if analyze_monitoring and (self.loki_collector or self.prometheus_collector):
            ...
            monitoring_dependencies = self.monitoring_discovery.create_dependencies_from_traffic_patterns(
                patterns=patterns,
                min_confidence=0.5
            )
    ...
    def _extract_connection_string_dependencies(
        ...
        connection_keys = [
            "connectionString", "connection_string", "connectionStrings",
            "databaseUrl", "database_url", "dbConnectionString",
            "storageConnectionString", "storage_connection_string",
            "redisConnectionString", "redis_connection_string",
            "cacheConnectionString", "cache_connection_string",
            "endpoint", "endpoints", "serviceUrl", "service_url",
            ...
            if any(conn_key in key.lower() for conn_key in connection_keys):
                if isinstance(value, str):
                    ...
                    if conn_info and conn_info.host:
                        target_id = self.connection_parser.extract_host_from_connection_info(conn_info)
        ...
        nested_keys = ["environment", "env", "appSettings", "app_settings", "config", "configuration"]
    ...
    def get_enrichment_capabilities(self) -> dict[str, bool]:
        ...
        return {
            ...
            "monitoring_analysis": (
                self.loki_collector is not None or
                self.prometheus_collector is not None
            )
        }
